...
function init() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    const tab = tabs[0];
    if (!tab || !tab.url) { showNoTicket(); return; }

    const urlMatch =
      tab.url.match(/\/browse\/([A-Z]+-\d+)/i) ||
      tab.url.match(/atlassian\.net.*?([A-Z]+-\d+)/i);

    if (!urlMatch) { showNoTicket(); return; }

    currentTicketId = urlMatch[1].toUpperCase();
    ...
    chrome.storage.local.get([key], (result) => {
      ...
      if (ticket) {
        ...
        // Show minimal panel even if content script hasn't fired yet
        showTicket({
...
function showTicket(ticket) {
  ...
  // Meta fields
  if (ticket.assignee) {
    document.getElementById('meta-assignee').style.display = 'flex';
    document.getElementById('assignee-text').textContent = ticket.assignee;
  }
  if (ticket.priority) {
    document.getElementById('meta-priority').style.display = 'flex';
    document.getElementById('priority-text').textContent = ticket.priority;
  }
  if (ticket.storyPoints) {
    document.getElementById('meta-points').style.display = 'flex';
...
function saveField(field, value) {
  ...
  chrome.storage.local.get([key], (result) => {
    ...
    ticket[field] = value;
    chrome.storage.local.set({ [key]: ticket });
  });
}
